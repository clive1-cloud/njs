...
const CustomXAxisTick: React.FC<any> = ({ x, y, payload }) => {
  return (
    // ✅ Fix 1: 'left' is not valid — changed to 'start'
...
export default function SalesAreaChart({ data }: { data: any[] }) {
  ...
  // ✅ Fix 2: Normalize theme so 'system' never breaks STROKE_COLORS lookup
  const resolvedTheme = theme === 'dark' ? 'dark' : 'light'
  const { cssColors, color } = useColorStore(resolvedTheme)
    ...
    <ResponsiveContainer width='100%' height={400}>
      <AreaChart data={data}>
        ...
        <Area
          type='monotone'
          dataKey='totalSales'
          stroke={STROKE_COLORS[color.name]?.[resolvedTheme]}
          strokeWidth={2}
